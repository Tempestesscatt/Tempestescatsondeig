...
# --- 1. OPTIMITZACIÓ DE MEMÒRIA CAU ---
@st.cache_data(ttl=1800, show_spinner="Carregant dades del sondeig...")
def carregar_sondeig_per_coordenades(lat, lon):
...
def processar_sondeig_per_hora(_sondeo, hourly_index, p_levels):  # Cambio clave aquí (_sondeo)
    try:
        ...
        s_idx, n_plvls = 3, len(p_levels)
        T_p, Td_p, Ws_p, Wd_p, H_p = ([hourly.Variables(s_idx + i*n_plvls + j).ValuesAsNumpy()[hourly_index] for j in range(n_plvls)] for i in range(5))
        ...
        def interpolate_sfc(sfc_val, p_sfc, p_api, d_api):
            ...
            if np.isnan(sfc_val) and len(valid_p) > 1:
                p_sorted, d_sorted = zip(*sorted(zip(valid_p, valid_d)))
                return np.interp(p_sfc, p_sorted, d_sorted)
            return sfc_val
        
        T_s = interpolate_sfc(T_s, P_s, p_levels, T_p)
        Td_s = interpolate_sfc(Td_s, P_s, p_levels, Td_p)
        ...
        for i, p_level in enumerate(p_levels):
            if p_level < P_s and not np.isnan(T_p[i]):
                p_profile.append(p_level)
                T_profile.append(T_p[i])
                Td_profile.append(Td_p[i])
                h_profile.append(H_p[i])
                u_comp, v_comp = mpcalc.wind_components(Ws_p[i]*units.knots, Wd_p[i]*units.degrees)
                u_profile.append(u_comp.to('m/s').m)
                v_profile.append(v_comp.to('m/s').m)
                
        return (
            np.array(p_profile)*units.hPa, 
            np.array(T_profile)*units.degC,
            np.array(Td_profile)*units.degC,
            np.array(u_profile)*units.m/units.s,
            np.array(v_profile)*units.m/units.s,
            np.array(h_profile)*units.m
        )
    except Exception as e:
        return None

@st.cache_data(ttl=1800, show_spinner="Calculant mapes de vents...")
def obtener_dades_mapa(variable, nivell, hourly_index):
    lats, lons = np.linspace(40.5, 42.8, 12), np.linspace(0.2, 3.3, 12)
    api_vars = []
    
    if variable == 'wind':
        api_vars = [f"wind_speed_{nivell}hPa", f"wind_direction_{nivell}hPa"]
    elif variable == 'temp_height':
    ...
    try:
        ...
        for r in responses:
            ...
            for i in range(len(api_vars)):
                val_array = hourly.Variables(i).ValuesAsNumpy()
                if hourly_index < len(val_array):
                    values.append(val_array[hourly_index])
                else:
...
@st.cache_data(ttl=3600, show_spinner="Calculant convergències...")
def calcular_convergencia_per_totes_les_localitats(_hourly_index, _nivell):
    lats_mapa, lons_mapa, data_mapa, error = obtener_dades_mapa('wind', _nivell, _hourly_index)
...
@st.cache_data(ttl=10800, show_spinner="Precalculant potencials diaris...")
def precalcular_potencials_del_dia():
    ...
    def procesar_poble(nom_poble, coords):
        sondeo, p_levels, _ = carregar_sondeig_per_coordenades(coords['lat'], coords['lon'])
        if sondeo:
            for hora in horas_muestreadas:
                ...
                if profiles:
                    ...
                    avis = generar_avis_potencial_per_precalcul(parametros)
                    if avis in avisos_a_buscar:
                        return nom_poble, hora
        return None
    
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(procesar_poble, nom, coord) for nom, coord in pobles_data.items()]
...
def get_next_arome_update_time():
    ...
    if next_update_time is None:
        next_update_time = (now_utc + timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0) + availability_delay
...
def calculate_parameters(p, T, Td, u, v, h):
    ...
    def get_val(qty, unit=None):
        try: 
            return qty.to(unit).m if unit else qty.m
        except: 
            return None
    ...
    try:
        ...
        if params.get('CAPE_Brut', {}).get('value', 0) > 0:
            ...
            params['CAPE_Utilitzable'] = {
                'value': max(0, params['CAPE_Brut']['value'] - abs(params.get('CIN_Fre', {}).get('value', 0))), 
                'units': 'J/kg'
            }
    except Exception as e:
        pass
    ...
    try:
        ...
        lfc_h = mpcalc.pressure_to_height_std(lfc_p)
    ...
    try:
        ...
        el_h = mpcalc.pressure_to_height_std(el_p)
    ...
    try:
        ...
        shear_magnitude = mpcalc.wind_speed(s_u, s_v)
        params['Shear_0-6km'] = {'value': get_val(shear_magnitude, 'm/s'), 'units': 'm/s'}
    except:
        pass
    
    try:
        _, srh, _ = mpcalc.storm_relative_helicity(h, u, v, depth=1*units.km)
        params['SRH_0-1km'] = {'value': get_val(srh), 'units': 'm²/s²'}
    except:
        pass
    
    try:
        pwat = mpcalc.precipitable_water(p, Td)
        params['PWAT_Total'] = {'value': get_val(pwat, 'mm'), 'units': 'mm'}
    except:
        pass
    ...
    try:
        ...
        T_interp = mpcalc.interpolate_1d(p_levels_interp, p, T)
    ...
    try:
        cape_val = params.get('CAPE_Utilitzable',{}).get('value',0)
        lcl_val = params.get('LCL_AGL',{}).get('value',9999)
        srh_val = params.get('SRH_0-1km',{}).get('value',0)
        shear_val = params.get('Shear_0-6km',{}).get('value',0)
        
        if all(v is not None for v in [cape_val, lcl_val, srh_val, shear_val]):
            stp_cin = mpcalc.significant_tornado(
                cape=cape_val*units('J/kg'), 
                lcl_height=lcl_val*units.m, 
                storm_helicity=srh_val*units('m^2/s^2'), 
                bulk_shear=shear_val*units('m/s')
            )
...
def display_avis_principal(titol_avís, text_avís, color_avís, icona_personalitzada=None):
    ...
    <div style="padding: 1rem; border-radius: 0.5rem; background-color: var(--secondary-background-color); border-left: 8px solid {color_avís}; box-shadow: 0 4px 15px rgba(0,0,0,0.1); margin-bottom: 1rem; display: flex; align-items: center; gap: 1rem;">
        ...
        <div>
            <h3 style="color: {color_avís}; margin-top: 0; margin-bottom: 0.5rem; font-weight: bold;">{titol_avís}</h3>
            <p style="margin-bottom: 0; color: var(--text-color);">{text_avís}</p>
        </div>
...
def generar_avis_temperatura(params):
    ...
    if temp > 36: 
        return "AVÍS PER CALOR EXTREMA", f"Es preveu una temperatura de {temp:.1f}°C. Risc molt alt.", "#FF0000", "🥵"
    if temp < 0: 
        return "AVÍS PER FRED INTENS", f"Es preveu una temperatura de {temp:.1f}°C. Risc de gelades fortes.", "#0000FF", "🥶"
...
def generar_avis_localitat(params, is_convergence_active):
    ...
    if cape_u < 100 or (cin is not None and cin < -150): 
    ...
    if cond_supercelula and stp > 0.8:
        ...
        if lcl_agl < 1000: 
            risks.insert(0, "tornados")
        risks_text = ", ".join(risks)
        if is_convergence_active: 
            return "RISC ALT", f"Entorn de SUPERCL·LULA TORNÀDICA (STP={stp:.1f}). La convergència forta actua com a disparador. Risc de {risks_text}.", "#E60073"
        else: 
            return "POTENCIAL SEVER", f"Potencial latent de SUPERCL·LULA TORNÀDICA (STP={stp:.1f}) per falta d'un disparador clar i fort. L'entorn és perillós.", "#FFC300"
    
    elif cond_avis_sever:
        risks = []
        if dcape > 900: 
            risks.append("fortes ràfegues de vent (esclafits severs)")
        if lr > 6.8: 
            risks.append("calamarsa de mida considerable")
        if pwat > 35: 
            risks.append("pluges torrencials i inundacions sobtades")
        elif not risks: 
            risks.append("fortes pluges")
        
        risks_text = ", ".join(risks)
        if is_convergence_active: 
            return "AVÍS", f"Potencial per a tempestes SEVERES organitzades. La convergència forta afavoreix el seu inici. Risc de {risks_text}.", "#FF8C00"
        else: 
            return "POTENCIAL MODERAT", f"Entorn de tempesta SEVERA latent per falta d'un disparador clar i fort. Risc de {risks_text} si s'activa.", "#FFD700"
    
    elif cond_precaucio:
        missatge_base = "Risc de TEMPESTES ORGANITZADES (multicèl·lules). Possibles fortes pluges i calamarsa local."
        if pwat > 35: 
            missatge_base += " Atenció al risc de xàfecs torrencials."
        if is_convergence_active:
...
def generar_analisi_detallada(params, is_convergence_active):
    ...
    if shear6 is not None:
        if shear6 < 10: 
            shear_text = "Molt feble, afavorint tempestes desorganitzades i de curta durada (unicel·lulars)."
        elif shear6 < 18: 
            shear_text = "Moderat, suficient per organitzar les tempestes en sistemes multicel·lulars o línies de tempestes."
        else: 
            shear_text = "Fort, un ingredient clau per al desenvolupament de supercèl·lules amb rotació (mesociclons)."
    ...
    if srh1 is not None and srh1 > 100:
        ...
        lcl_text = f"Especialment si la base del núvol (LCL) és baixa com en aquest cas ({lcl_agl:.0f} m), " if lcl_agl and lcl_agl < 1200 else ""
    ...
    if stp and stp > 0.5: 
        yield from stream_text(f"**Potencial Tornàdic (STP):** L'índex de tornado significatiu és de **{stp:.1f}**. Valors superiors a 1 indiquent un entorn molt favorable per a supercèl·lules tornàdiques si s'arriben a formar.")
    if dcape and dcape > 800: 
        yield from stream_text(f"**Potencial de Vent Sever (DCAPE):** El valor de **{dcape:.0f} J/kg** indica un alt potencial per a la formació de corrents descendents molt forts (esclafits o 'downbursts') que poden causar danys a la superfície.")
    if pwat and pwat > 30: 
        yield from stream_text(f"**Potencial de Precipitació Intensa (Aigua Precipitable):** El contingut d'humitat a la columna atmosfèrica és elevat ({pwat:.1f} mm), afavorint xàfecs de gran intensitat i possibles inundacions locals.")
    
    yield from stream_text(f"**La Clau del Pronòstic:** La clau principal avui és la interacció entre la **inestabilitat {cape_text}** i un **cisallament {('fort' if shear6 and shear6>18 else 'moderat')}**. La presència (o absència) d'un mecanisme de tret com la **convergència forta** serà el factor decisiu per determinar si s'allibera aquest potencial i quin tipus de tempestes es desenvolupen.")

def display_metrics(params_dict):
    param_map = [
        ('Temperatura','SFC_Temp'), 
        ('CAPE Utilitzable','CAPE_Utilitzable'), 
        ('CIN (Fre)','CIN_Fre'), 
        ('Vel. Asc. Màx.','W_MAX'),
        ('Shear 0-6km','Shear_0-6km'), 
        ('SRH 0-1km','SRH_0-1km'), 
        ('Potencial Tornàdic','STP_cin'), 
        ('Potencial Esclafits','DCAPE'),
        ('Gradient Tèrmic','LapseRate_700_500'), 
        ('Aigua Precipitable','PWAT_Total'), 
        ('Base núvol (AGL)','LCL_AGL'), 
        ('Cim tempesta (MSL)','EL_MSL')
    ]
    
    st.markdown("""<style>
        .metric-container{border:1px solid rgba(128,128,128,0.2);border-radius:10px;padding:10px;margin-bottom:10px;}
...
def crear_mapa_generic(lats, lons, data, nivell, lat_sel, lon_sel, nom_poble_sel, titol_var, cmap, unitat, levels):
    ...
    X, Y = np.meshgrid(grid_lon, grid_lat)
    points = np.vstack((lons, lats)).T
    grid_data = griddata(points, data, (X, Y), method='cubic')
    grid_data = np.nan_to_num(grid_data)
    cont = ax.contourf(X, Y, grid_data, cmap=cmap, levels=levels, alpha=0.7, zorder=2, transform=ccrs.PlateCarree(), extend='both')
    fig.colorbar(cont, ax=ax, orientation='vertical', label=f'{titol_var} ({unitat})', shrink=0.7)
    return fig
...
def crear_mapa_temp_isobares(lats, lons, data, nivell, lat_sel, lon_sel, nom_poble_sel):
    ...
    X, Y = np.meshgrid(grid_lon, grid_lat)
    points = np.vstack((lons, lats)).T
    grid_temp = griddata(points, temps, (X, Y), method='cubic')
    grid_temp = np.nan_to_num(grid_temp)
    grid_height = griddata(points, heights, (X, Y), method='cubic')
    grid_height = np.nan_to_num(grid_height)
...
def crear_hodograf(p, u, v, h):
    ...
    hodo = Hodograph(ax, component_range=40.)
    hodo.add_grid(increment=10)
    hodoline = hodo.plot_colormapped(u, v, h.to('km'), cmap='gist_ncar')
    plt.colorbar(hodoline, ax=ax, orientation='vertical', pad=0.05, shrink=0.8).set_label('Altitud (km)')
    
    try:
        rm, _, _ = mpcalc.bunkers_storm_motion(p, u, v, h)
        hodo.plot_vectors(rm[0].to('kt'), rm[1].to('kt'), color='black', label='Mov. Tempesta (RM)')
    except:
        pass
...
def crear_grafic_orografia(params, zero_iso_h_agl):
    ...
    ax.annotate("", xy=(mountain_lfc_intersect_x, lfc_km if has_lfc else peak_h_km), 
                xytext=(1, 0.1), 
                arrowprops=dict(arrowstyle="->", color='white', lw=2.5, linestyle=':', connectionstyle="arc3,rad=0.2"), 
                zorder=4)
    ...
    if has_lfc:
        if peak_h_m >= lfc_agl:
            ...
            analysis_text = ax.text(5, max(peak_h_km, lfc_km) + 1, 
                                    f"DISPARADOR OROGRÀFIC POTENCIAL!\nUna muntanya de {lfc_agl:.0f} m o més seria suficient.", 
    ...
    y_label_text = ax.text(0.2, final_ylim - 0.2, "Altitud (km)", ha='left', va='top', color='white', weight='bold', fontsize=12)
    y_label_text.set_path_effects(main_text_effect)
    
    for y_tick in ax.get_yticks():
        if y_tick > 0 and y_tick < final_ylim:
            tick_label = ax.text(0.15, y_tick, f'{int(y_tick)}', ha='left', va='center', color='white', weight='bold', fontsize=9)
...
def crear_grafic_nuvol(params, H, u, v, is_convergence_active):
    ...
    if is_convergence_active and cape > 100:
        ...
        for y, width in zip(y_points, cloud_width):
            ...
            for _ in range(25):
                x_pos = center_x + (random.random() - 0.5) * width
                y_pos = y + (random.random() - 0.5) * 0.4
                size = 0.2 + random.random() * 0.4
                ax.add_patch(Circle((x_pos, y_pos), size, color='white', alpha=0.15, lw=0))
        ...
        for _ in range(80):
            x_pos = anvil_center_x + (random.random() - 0.2) * 4 + anvil_wind_spread
            y_pos = el_km + (random.random() - 0.5) * 0.5
            size = 0.2 + random.random() * 0.6
            ax.add_patch(Circle((x_pos, y_pos), size, color='white', alpha=0.2, lw=0))
        
        if cape > 2500:
            ax.add_patch(Circle((anvil_center_x, el_km + cape / 5000), 0.4, color='white', alpha=0.5))
    else:
        ax.text(center_x_base, 8, "Sense disparador o energia\nsuficient per a convecció profunda.", 
    ...
    barb_heights_m = barb_heights_km * 1000
    u_barbs = np.interp(barb_heights_m, H.m, u.to('kt').m)
    v_barbs = np.interp(barb_heights_m, H.m, v.to('kt').m)
...
# Obtenim l'hora seleccionada
hourly_index_global = int(st.session_state.hora_selector.split(':')[0])

# Càlcul de convergències
with st.spinner("Calculant convergències per a tot el territori..."):
    convergencies_850hpa = calcular_convergencia_per_totes_les_localitats(hourly_index_global, 850)
...
# Precalcul de potencials
with st.spinner("Analitzant potencials diaris..."):
    potencials_detectats_avui = precalcular_potencials_del_dia()
...
with st.expander("⚡️ Avisos i Potencials Detectats Avui", expanded=st.session_state.avisos_expanded):
    if not potencials_detectats_avui:
        ...
        for poble, hora in potencials_detectats_avui.items():
            convergencies_hora_potencial = calcular_convergencia_per_totes_les_localitats(hora, 850)
        ...
        with col_disparador:
            ...
            if not pobles_amb_disparador:
                st.write("Cap localitat amb potencial i disparador fort detectat.")
            for poble, hora in pobles_amb_disparador:
                st.button(f"{poble} (a les {hora:02d}:00h)", key=f"btn_disparador_{poble}_{hora}", on_click=actualitzar_seleccio, args=(poble, hora))
        
        with col_potencial:
            st.markdown("##### 🔎 Inestable (Sense convergència forta)")
            if not pobles_amb_potencial:
                st.write("Totes les localitats amb potencial tenen un disparador fort.")
            for poble, hora in pobles_amb_potencial:
                st.button(f"{poble} (a les {hora:02d}:00h)", key=f"btn_potencial_{poble}_{hora}", on_click=actualitzar_seleccio, args=(poble, hora))

# PROCESSAMENT I VISUALITZACIÓ SOTA DEMANDA
with st.spinner(f"Carregant sondeig detallat per a {poble_sel}..."):
    sondeo, p_levels, error_sondeo = carregar_sondeig_per_coordenades(lat_sel, lon_sel)
...
if error_sondeo:
    ...
    try:
        ...
        if profiles:
            ...
            # Comprovem si el disparador està actiu
            ...
            with tab_mapes:
                ...
                with col_tipus:
                    selected_map_name = st.selectbox("Tipus de mapa:", list(map_options.keys()))
                ...
                with st.spinner(f"Generant mapa de {selected_map_name.lower()}..."):
                    ...
                    lats, lons, data, error = obtener_dades_mapa(api_var, nivell_global, hourly_index)
                    ...
                    if error:
                        ...
                        if selected_map_name == "Vents i Convergència":
                            ...
                            fig = crear_mapa_generic(
                                lats, lons, data, nivell_global, lat_sel, lon_sel, poble_sel, 
                                map_config["titol"], map_config["cmap"], map_config["unitat"], map_config["levels"]
                            )
