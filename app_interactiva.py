...
def crear_grafic_perfil_orografic(analisi, params_calc, layer_to_show, max_alt_m, show_barbs=True):
    ...
    Versió 14.0 (Núvols amb transparència difuminada):
    - Els núvols es dibuixen amb alpha gradual segons HR (80–100%).
    - S’aplica suavitzat gaussià per evitar ratlles artificials.
    - Si hi ha CAPE > 100, es dibuixa una columna convectiva central.
    """
    ...
    # --- Configuració de paletes ---
    levels_theta_e = list(range(15, 75, 5))
    cmap_theta_e = plt.get_cmap('viridis')
    ...
    colors_vent = ['#d3d3d3', '#add8e6', '#48d1cc', '#90ee90', '#32cd32',
                   '#6b8e23', '#f0e68c', '#d2b48c', '#bc8f8f', '#ffb6c1',
                   '#da70d6', '#9932cc', '#8a2be2']
    ...
    colors_temp = ['#8a2be2', '#0000ff', '#1e90ff', '#00ffff', '#32cd32',
                   '#ffff00', '#ffa500', '#ff0000', '#dc143c', '#ff00ff']
    ...
    # Graella deformada per l’orografia
    x_grid = dist_centrat
    ...
    if layer_to_show == "Humitat":
        ...
        data_grid = np.interp(zz_deformat, heights_m, profile_celsius)
        cmap, norm, levels, label = cmap_theta_e, norm_theta_e, levels_theta_e, "Theta-E (°C)"
    ...
    # --- Pintat de capa (excepte núvols) ---
    if data_grid is not None:
        masked_data = np.where(zz_asl > elev, data_grid, np.nan)
        im = ax.contourf(xx, zz_asl, masked_data, levels=levels, cmap=cmap, norm=norm,
                         extend='both', zorder=1)
        contours = ax.contour(xx, zz_asl, masked_data, levels=levels[1:-1:2],
                              colors='black', linewidths=0.5, alpha=0.7, zorder=2)
        ax.clabel(contours, inline=True, fontsize=7, fmt='%1.0f')
        fig.colorbar(im, ax=ax, label=label, pad=0.02, ticks=levels[::2])

    # --- Núvols amb transparència ---
    if layer_to_show == "Núvols":
        sky_gradient = np.linspace(0.8, 0.4, 100).reshape(-1, 1)
        ax.imshow(sky_gradient, aspect='auto', cmap='Blues_r', origin='lower',
                  extent=[dist_centrat.min(), dist_centrat.max(), 0, max_alt_m], zorder=0)

        rh_grid = np.interp(zz_asl, heights_m, rh_profile)

        # Alpha segons HR (80–100%), sense gaussian_filter
        cloud_alpha = np.clip((rh_grid - 80) / 20, 0, 1)

        ax.imshow(cloud_alpha, extent=[dist_centrat.min(), dist_centrat.max(), 0, max_alt_m],
                  origin='lower', aspect='auto', cmap='Greys', alpha=cloud_alpha, zorder=2)

        # Convecció si hi ha CAPE
        cape = params_calc.get('MLCAPE', 0)
        if cape > 100:
            ...
            if el > lcl:
                ...
                ax.contourf(xx, zz_asl, convective_mask.astype(float),
                            levels=[0.5, 1.5], colors=['#FFFFFF'], alpha=0.85, zorder=3)
    ...
    if np.min(elev) <= 5:
        ...
        ax.fill_between(x_wave, -100, y_wave, where=y_wave > 0,
                        color='#6495ED', alpha=0.6, zorder=2)
    ...
    if level_hgt is not None and level_hgt < max_alt_m:
        ax.axhline(y=level_hgt, color='white', linestyle=':', linewidth=2,
                   label=f"{level_label}: {level_hgt:.0f} m", zorder=4,
    ...
    if show_barbs:
        ...
        ax.barbs(barb_xx.flatten()[valid_mask_upper], barb_zz.flatten()[valid_mask_upper],
                 barb_u[valid_mask_upper], barb_v[valid_mask_upper], length=6, zorder=5,
        ...
        ax.barbs(barb_x_surface[mask], barb_y_surface[mask],
                 barb_u_surface[mask], barb_v_surface[mask], length=6, zorder=5,
                 color='#F0E68C', path_effects=[path_effects.withStroke(linewidth=2, foreground='black')])
        for x, y_terrain, y_barb in zip(barb_x_surface[mask], surface_elev_at_barbs[mask], barb_y_surface[mask]):
            ax.plot([x, x], [y_terrain, y_barb], color='#F0E68C', lw=0.6,
                    linestyle='--', zorder=4)

    poble_dist_centrat = analisi['poble_dist'] - (dist_total_km / 2)
    ax.plot(poble_dist_centrat, analisi['poble_elev'], 'o', color='red',
            markersize=8, label=f"{analisi['poble_sel']} ({analisi['poble_elev']:.0f} m)",
