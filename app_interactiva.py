...
def processar_dades_sondeig(p_profile, T_profile, Td_profile, u_profile, v_profile, h_profile):
    """
    Versió Definitiva i Antifràgil v5.4 - CORRECCIÓ LCL/LFC.
    Utilitza la funció d'interpolació de MetPy (interpolate_1d) per a un càlcul
    robust de les alçades LCL, LFC i EL, solucionant errors de NaN.
    """
    ...
    # --- 2. CÀLCULS BASE I PERFILS DE PARCEL·LA (Sense canvis) ---
    with parcel_lock:
        ...
        # (La majoria de càlculs es mantenen igual)
        try: 
        ...
        # --- BLOC DE L'LFC, LCL i EL TOTALMENT CORREGIT ---
        if main_prof is not None:
            ...
            try:
                ...
                # --- CANVI CLAU: Utilitzem la interpolació de MetPy ---
                lfc_hgt_interpolated = mpcalc.interpolate_1d(lfc_p, p, heights_agl)
                params_calc['LFC_Hgt'] = float(lfc_hgt_interpolated.m)
            except: params_calc.update({'LFC_p': np.nan, 'LFC_Hgt': np.nan})
            try:
                el_p, _ = mpcalc.el(p, T, Td, main_prof); params_calc['EL_p'] = float(el_p.m)
                # --- CANVI CLAU: Utilitzem la interpolació de MetPy ---
                el_hgt_interpolated = mpcalc.interpolate_1d(el_p, p, heights_agl)
                params_calc['EL_Hgt'] = float(el_hgt_interpolated.m)
        ...
        try:
            ...
            # --- CANVI CLAU: Utilitzem la interpolació de MetPy ---
            lcl_hgt_interpolated = mpcalc.interpolate_1d(lcl_p, p, heights_agl)
            params_calc['LCL_Hgt'] = float(lcl_hgt_interpolated.m)
