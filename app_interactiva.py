...
def processar_dades_sondeig(p_profile, T_profile, Td_profile, u_profile, v_profile, h_profile):
    """
    Versión Simplificada y Robustecida - Solo cálculos esenciales
    """
    try:
        # Validación básica
        if len(p_profile) < 3:
        ...
        # 2. PERFIL DE PARCELA SUPERFICIAL
        try:
            sfc_prof = mpcalc.parcel_profile(p, T[0], Td[0])
        except:
            sfc_prof = None
        
        # 3. CAPE/CIN SUPERFICIAL (el más importante)
        ...
        # 5. VENTO Y CIZALLAMIENTO (si hay datos)
        ...
        return ((p, T, Td, u, v, heights, None), params_calc), None
